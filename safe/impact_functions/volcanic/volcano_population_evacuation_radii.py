...
class VolcanoFunctionEvacuationZones(FunctionProvider):
    ...
    def run(self, layers):
        ...
        Input
          layers: List of layers expected to contain
              H: Raster layer of volcano location
              P: Raster layer of population data on the same grid as H

        Counts number of people inside circular evacuation zone.
        ...
        # Check that hazard is point type
        if not H.is_vector:
            msg = ('Input hazard %s  was not a vector layer as expected '
                   % H.get_name())
            raise InaSAFEError(msg)

        msg = ('Input hazard must be a point layer. I got %s with layer '
               'type %s' % (H.get_name(),
                            H.get_geometry_name()))
        if not H.is_point_data:
            raise InaSAFEError(msg)

        msg = ('Input hazard must contain one point only. I got %i points '
               % len(H))
        if len(H) != 1:
            raise InaSAFEError(msg)
        ...
        Z.write_to_file('Marapi_evac_zone.shp')

        # Run interpolation function for polygon2raster
        P = assign_hazard_values_to_exposure_data(Z, E,
                                             attribute_name='population')

        # Count population inside circle
        evacuated = 0
